let body,
    ...
    stats,
    contentElement,
    colors = ['#8ecae6', '#219ebc', '#023047', '#6497b1', '#005b96'],
    graphics,
    currentGraphic = 0,
    graphicCanvas,
    gctx,
...
const initStage = () => {
    ...
    contentElement = document.querySelector('.image-content');
    setWindowSize();
...
const initCanvas = () => {
    ...
    graphicCanvas.willReadFrequently = true;
    graphicCanvas.width = canvasWidth;
    graphicCanvas.height = canvasHeight;
    gctx = graphicCanvas.getContext('2d');
    graphics = document.querySelectorAll('.image-cell > img');
};

function update(x) {
    let num = parseInt(x.id);
    if (currentGraphic != num) {
        updateGraphic(num);
        currentGraphic = num;
    }
};
...
const initLights = () => {
    const shadowLight = new THREE.DirectionalLight(0x000000, 2);
    shadowLight.position.set(20, 0, 10);
    scene.add(shadowLight);

    const light = new THREE.DirectionalLight(0x000000, 1.5);
...
const updateGraphic = (num) => {
    const img = graphics[num];
...
const elem = document.querySelector('.image-carousel');
initStage();
initScene();
initCanvas();
initCamera();
